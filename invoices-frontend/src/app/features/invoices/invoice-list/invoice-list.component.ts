...
export class InvoiceListComponent implements OnInit {
  ...
  submitPayment() {
    ...
    if (inv.paymentStatus === PaymentStatus.Draft) {
      this.toast.error('Draft invoices cannot accept payments');
      return;
    }
    if (amount > inv.remainingAmount) {
      this.toast.error('Payment cannot exceed the remaining amount');
      return;
    }
