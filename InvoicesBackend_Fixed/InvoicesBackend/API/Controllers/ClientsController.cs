...
public class ClientsController : ControllerBase
{
    ...
    public ClientsController(ApplicationDbContext context, PlanGuardService planGuard)
    {
        _context = context;
    ...
    public async Task<IActionResult> CreateClient(UpdateClientRequest request)
    {
        ...
        if (Guid.TryParse(uidClaim, out var ownerUid))
        {
            var limitErr = await _planGuard.CheckClientLimitAsync(business.Id, ownerUid);
            if (limitErr != null) return BadRequest(limitErr);
        }
